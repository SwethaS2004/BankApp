...
public class DepositServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (customerDAO.updateBalance(accountNumber, amount)) {
                if (customerDAO.insertTransaction(accountNumber, amount, "Deposit")) {
                    conn.commit();
                    ...
                    conn.rollback();
                    request.getSession().setAttribute("message", "Deposit failed. Please try again.");
                }
            } else {
                conn.rollback();
                request.getSession().setAttribute("message", "Deposit failed. Please try again.");
